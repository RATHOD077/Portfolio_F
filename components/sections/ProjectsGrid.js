...
import { ExternalLink, Github, Star, Globe, Smartphone, Zap, Rocket, Lightbulb, Terminal, Code2 } from 'lucide-react';
...
function CyberCard({ children }) {
  ...
  return (
    <div
      ...
      className="relative h-full flex flex-col group [perspective:1000px] w-full"
    >
      <motion.div
    ...
    <CyberCard>
      ...
      <div className="flex flex-col flex-1 p-6 bg-[#020817] rounded-b-2xl z-10">
        ...
        <p className="text-sm text-white/50 leading-relaxed flex-1 line-clamp-3 font-medium">
          {stripHtml(project.description || project.short_desc)}
        </p>
...
const SkeletonCard = () => (
  <div className="rounded-2xl overflow-hidden border border-white/5 bg-[#0a0f1e] h-[400px] flex flex-col min-w-[300px] md:min-w-[380px] flex-shrink-0">
...
export default function ProjectsGrid({ limit = undefined } = {}) {
  ...
  if (loading) return (
    <div className="flex gap-6 md:gap-8 overflow-x-hidden py-4">
      {[0, 1, 2].map(i => <SkeletonCard key={i} />)}
    </div>
  );

  if (projects.length === 0) return (
    <div className="text-center py-24 flex flex-col items-center gap-4 bg-[#0a0f1e] border border-white/10 rounded-3xl">
  ...
  return (
    <div className="relative w-full overflow-hidden py-6">
      ...
      <div className="absolute left-0 top-0 bottom-0 w-24 bg-gradient-to-r from-[#020817] to-transparent z-20 pointer-events-none" />
      <div className="absolute right-0 top-0 bottom-0 w-24 bg-gradient-to-l from-[#020817] to-transparent z-20 pointer-events-none" />

      {/* Infinite Scrolling Track */}
      <div className="flex w-full overflow-x-hidden">
        <div className="marquee-container gap-6 md:gap-8 pr-6 md:pr-8">
          {marqueeItems.map((p, i) => (
            <div 
              key={`${p.id}-${i}`} 
              className="w-[280px] sm:w-[320px] md:w-[380px] lg:w-[400px] flex-shrink-0"
            >
